...
export default function EditArticle({ article }: EditArticleProps) {
  ...
  const [imagePreview, setImagePreview] = useState<string | null>(article?.featuredImageUrl || null);
  ...
  // Watch for server action errors
  useEffect(() => {
  ...
  const handleImageChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (file.size > 5 * 1024 * 1024) {
        ...
        e.target.value = "";
        setImagePreview(article?.featuredImageUrl || null);
        return;
      }
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    startTransition(() => {
      action(formData);
      toast.success("Updating article...");
    });
  };
